...
def load_grouping_old(filename, delimiter=',', skip_col=0):
    group_list = {}
    group_dict = {}
    ...
    with open(filename) as f:
        l = f.readline()
        tmp = l.strip('\n').split(delimiter)
        group_list = tmp[skip_col:len(tmp)]
            
    return group_list

def load_grouping(filename, delimiter=','):
    group_list = []
    group_dict = {}
    ...
    with open(filename) as f:
        l = f.readline()
        group_list = l.strip('\n').split(delimiter)
        group_dict[group_list[0]] = group_list[1:len(group_list)]
    return group_dict
...
if args.matrix[0] is not None :
    matrix_path = args.matrix[0]
else :
    print("Missing -im or --matrix\n")
    exit()
if args.gene[0] is not None :
    gene_path = args.gene[0]
else :
    print("Missing -ig or --gene\n")
    exit()
...
if args.group is not None:
    grouping = args.group[0]
    if os.path.isfile(grouping):
        if grouping.lower().endswith('.tsv'):
            gsep = '\t'
	
        elif grouping.lower().endswith('.csv'):
	    gsep=','

        else:
            print("Cell Grouping file not found.")
            exit()
        # nombre de colonne a sauter 1
        cgroup = load_grouping(grouping,gsep)
        for k,labels in cgroup.items():
            print(k+" "+" ".join(labels[1:6])+"\n")

else:
    print("INFO : No Cell Grouping file provided.")
...
if args.group is None :
    ...
    out = make_spring_subplot(E, gene_list, save_path, 
                    ...
                    min_counts = 3, min_cells = 3, min_vscore_pctl = 85,show_vscore_plot = False, 
                    num_pc = 30, 
                    k_neigh = 4, cell_groupings = cgroup, 
                    num_force_iter = 500)
